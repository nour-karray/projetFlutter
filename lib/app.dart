...
/// Développement : si le mail de vérification Firebase n’arrive pas, lance avec
/// `flutter run ... --dart-define=SKIP_EMAIL_VERIFY=true` pour accéder à l’app.
/// En production, ne pas passer ce flag (valeur par défaut : false).
const bool _kSkipEmailVerification = bool.fromEnvironment(
  'SKIP_EMAIL_VERIFY',
  defaultValue: false,
);
...
class _SmartScanAppState extends State<SmartScanApp> {
  ...
  void _handleAuthStateChanged() {
    // Toujours rafraîchir le shell : ex. après vérification e-mail le uid est
    // inchangé, donc [_loadHistoryForCurrentUser] sort tôt sans setState sinon.
    if (mounted) setState(() {});
  ...
  Widget _buildHome() {
    ...
    if (!_authService.isEmailVerified && !_kSkipEmailVerification) {
