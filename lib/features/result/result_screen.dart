...
class _ResultScreenState extends State<ResultScreen> {
  ...
  Future<void> _analyzeAndGeneratePdf() async {
    ...
    if (report == null || report.documentType != DocumentType.medicalReport) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_medicalOnlyMessage())),
      );
      return;
    }
    ...
    final text = (_useTranslatedForGemini && translated.isNotEmpty) ? translated : rawText;
    if (text.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      if (!mounted) return;
      final extractedEntities = await medicalAiService.extractMedicalReportEntities(text);
      if (!mounted) return;
      ...
      if (existingReport != null) {
        final updatedReport = existingReport.copyWith(
          patientInfo: existingReport.patientInfo.copyWith(
            fullName: extractedEntities.patientName ?? existingReport.patientInfo.fullName,
            codePatient: extractedEntities.codePatient ?? existingReport.patientInfo.codePatient,
            organism: extractedEntities.organism ?? existingReport.patientInfo.organism,
          ),
          doctorInfo: existingReport.doctorInfo.copyWith(
            requesterName: extractedEntities.requesterDoctor ?? existingReport.doctorInfo.requesterName,
            validatorName: extractedEntities.validatorDoctor ?? existingReport.doctorInfo.validatorName,
          ),
          confidence: (existingReport.confidence + extractedEntities.confidence) / 2,
        );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${AppLocalizations.t(_lc, 'errorGeminiAnalysis')}: $e'),
        ),
      );
    } finally {
      if (mounted) {
  ...
  Future<void> _translateOcrText(String targetLanguageCode) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Erreur traduction: $e')),
      );
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<MedicalReport> _translateStructuredReport({
    ...
    final texts = <String>[
      ...
      ...report.sections.expand((s) => s.analyses).map((a) => a.referenceText ?? ''),
      ...report.sections.expand((s) => s.analyses).map((a) => a.technique ?? ''),
      ...report.sections.expand((s) => s.analyses).map((a) => a.interpretation ?? ''),
    ];
    ...
    final translatedPatient = report.patientInfo.copyWith(
      organism: next(),
    );
    final translatedDoctor = report.doctorInfo.copyWith(
      validatorName: next(),
    );
        ...
        builder: (context, _) {
          ...
          return Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Text('${AppLocalizations.t(_lc, 'detectedLanguage')}: '),
                  Chip(
                    label: Text(
              ...
              Wrap(
                ...
                children: widget.scanResultStore.detectedLanguageCodes
                    .map(
                      (code) => Chip(
                        label: Text(AppLocalizations.detectedLanguageLabel(code)),
                      ),
                    )
                    .toList(),
              ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          FilledButton.tonal(
                            onPressed: _isTranslating ? null : () => _translateOcrText('fr'),
                            child: const Text('FR'),
                          ),
                          FilledButton.tonal(
                            onPressed: _isTranslating ? null : () => _translateOcrText('en'),
                            child: const Text('EN'),
                          ),
                          FilledButton.tonal(
                            onPressed: _isTranslating ? null : () => _translateOcrText('ar'),
                            child: const Text('AR'),
                          ),
                        ],
                      ),
                            ...
                            OutlinedButton.icon(
                              onPressed: () async {
                                ...
                                messenger.showSnackBar(
                                  const SnackBar(content: Text('Traduction copiee.')),
                                );
                              },
              ...
              if (viewReport != null) ...[
                ...
                MedicalInfoCard(
                  title: AppLocalizations.t(_lc, 'medicalPatientInfoTitle'),
                  lines: [
                ...
                ...viewReport.sections.map((section) => MedicalSectionCard(section: section)),
                const SizedBox(height: 16),
                const Divider(),
                const SizedBox(height: 8),
                Card(
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: FilledButton.icon(
                            ...
                            label: Text(AppLocalizations.t(_lc, 'exportPdf')),
                          ),
                        ),
                        const SizedBox(height: 12),
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            ...
                            label: Text(AppLocalizations.t(_lc, 'viewRawOcr')),
                          ),
                        ),
                      ],
                    ),
              ...
              if (_showAiOptionalSection)
                Card(
                  ...
                  child: ExpansionTile(
                    ...
                    title: Text(AppLocalizations.t(_lc, 'geminiOptionalSectionTitle')),
                    subtitle: Text(
                      AppLocalizations.t(_lc, 'geminiOptionalSectionSubtitle'),
                      maxLines: 2,
                    ...
                    children: [
                      if (widget.geminiApiKey.trim().isEmpty)
                        ListTile(
                          ...
                          title: Text(AppLocalizations.t(_lc, 'geminiKeyRequiredTitle')),
                          subtitle: Text(AppLocalizations.t(_lc, 'geminiKeyRequiredSubtitle')),
                        ),
                      if (_isAnalyzing) const LinearProgressIndicator(),
                      if (widget.geminiApiKey.trim().isNotEmpty) ...[
                        const SizedBox(height: 8),
                        if (viewReport == null || viewReport.documentType != DocumentType.medicalReport)
                          ListTile(
                            contentPadding: EdgeInsets.zero,
                            leading: Icon(Icons.info_outline, color: colorScheme.primary),
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            onPressed: _isAnalyzing ||
                                    viewReport == null ||
                                    viewReport.documentType != DocumentType.medicalReport
                                ? null
                                : _analyzeAndGeneratePdf,
                            icon: const Icon(Icons.health_and_safety_outlined),
                            label: Text(
                              _isAnalyzing
                                  ? AppLocalizations.t(_lc, 'analyzingGemini')
                                  : AppLocalizations.t(_lc, 'analyzeGeminiPdf'),
                            ),
                          ),
                        ),
                      ],
                      if (_analysisResult != null) ...[
                        ...
                        const Divider(),
                        const SizedBox(height: 8),
                        _GeminiAnalysisCards(localeCode: _lc, analysis: _analysisResult!),
                      ],
                    ],
                  ),
                ),
